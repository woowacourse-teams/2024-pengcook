...
class CollectionFilteringHandlerTest {
    ...
    void canHandle() {
        List<Ownable> list = new ArrayList<>(List.of(new BlockingTestEntity(1L)));
        Set<Ownable> set = new HashSet<>(Set.of(new BlockingTestEntity(1L)));
    ...
    void handleFilteringWithListWhenBlocked() {
        List<Ownable> list = new ArrayList<>(List.of(new BlockingTestEntity(1L), new BlockingTestEntity(2L)));
        ...
        assertAll(
                ...
                () -> assertThat(list).containsExactly(new BlockingTestEntity(2L))
        );
    }
    ...
    void handleFilteringWithListWhenNotBlocked() {
        ...
        assertAll(
                () -> assertThat(result).isSameAs(list),
                () -> assertThat(list).containsExactly(new BlockingTestEntity(1L), new BlockingTestEntity(2L))
        );
    }
    ...
    void handleFilteringWithSetWhenBlocked() {
        Set<Ownable> set = new HashSet<>(Set.of(new BlockingTestEntity(1L), new BlockingTestEntity(2L)));
        User blockedUser = User.builder()
                .id(1L)
                .build();
        BlackList blackList = new BlackList(Set.of(blockedUser));
        ...
        assertAll(
                ...
                () -> assertThat(set).containsExactly(new BlockingTestEntity(2L))
        );
    }
    ...
    void handleFilteringWithSetWhenNotBlocked() {
        ...
        assertAll(
                () -> assertThat(result).isSameAs(set),
                () -> assertThat(set).containsExactly(new BlockingTestEntity(1L), new BlockingTestEntity(2L))
        );
    }
}
